...
void gotpkt(struct puldb* mydb,const struct pcap_pkthdr* header,const u_char* pkt)
{
	...
	else
	{
		...
		fprintf(stderr,"\n**  oops! looks like the wrong gateway was guessed..");
		fprintf(stderr,"\n**  trying alternate gateway %02x:%02x:%02x:%02x:%02x:%02x",
			mydb->gwmacalt[0],mydb->gwmacalt[1],mydb->gwmacalt[2],mydb->gwmacalt[3],
			mydb->gwmacalt[4],mydb->gwmacalt[5]);
		
		mydb->head=0;
		
		memcpy(mydb->gwmac,mydb->gwalt,6);
		strncpy(mydb->gwalt,"xxxxxx",6); // x out the alternate
	}
}
